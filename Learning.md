...
- binary search has to be sorted all the time
- the nodes might also have children
- two children at most
- at the root or on a node, the left is all smaller than the root and all greater than the right on the right.
...
- AVL trees are BST trees not all BST trees are AVL trees
- makes a single or double rotation on an unbalanced tree
